...
<header class="header">
    <div class="navi-box">
        <div class="brandLogo commoner">
            <a href="{{ route('index') }}">
<img src="{{ asset('images/logo.png') }}" style="width: 60px;height: 60px;">
</a>
</div>
<div class="ctgry commoner">
    <a href="#" class="dropdown">Category <i class="fa fa-angle-down" aria-hidden="true"></i></a>
    <ul class="catList">
        @foreach (App\Models\Category::orderBy('id', 'asc')->where('parent_id', NULL)->get() as $parent)
        <li>
            <a href="#{{ $parent->id }}">{{ $parent->name }}</a>
            <ul class="subList">
                @foreach (App\Models\Category::orderBy('id', 'asc')->where('parent_id', $parent->id)->get() as
                $child)
                <li class="subCat">
                    <a class="subCatitem" href="{!! route('categories.show', $child->id) !!}">{{ $child->name }}</a>
                </li>
                @endforeach
            </ul>
        </li>
...
<div class="searchHere commoner">
    <form class="" action="{!! route('search') !!}" method="get">
        <div class="form-group">
            <input type="text" class="navbar-form search" name="search" placeholder="Search Products" required="required">
            <button type="submit" class="btn searchBtn"><i class="fa fa-search" aria-hidden="true"></i></button>
        </div>
    </form>
</div>
<div class="lgnArea commoner">
    @if (!Auth::check())
    <a href="{{ route('login') }}" class="LnG">
        Login & Signup
    </a>
    @else
    <nav class="accArea">
        <a href="#" class="username">{{ Auth::user()->first_name }}</a>
        <ul class="accmenu">
            <li>
                <a href="{{ route('user.dashboard') }}" class="list">
                    <i class="fa fa-user icon" aria-hidden="true"></i>
                    My Profile
                </a>
            </li>
            <li>
                <a href="{{ route('user.profile') }}" class="list">
                    <i class="fa fa-cogs icon" aria-hidden="true"></i>
                    My Account
                </a>
            </li>
            <li class="cartArea">
                <a href="{{ route('carts') }}" class="list">
            ...
            <li>
                <a href="{{ route('wishlists') }}" class="list">
                    <i class="fa fa-heart icon" aria-hidden="true"></i>
                    My wishlist
                </a>
            </li>
            <li>
                <a href="{{ route('user.orders') }}" class="list">
                    <i class="fa fa-th-list icon" aria-hidden="true"></i>
                    My Orders
                </a>
            </li>
            <li><a href="{{ route('logout') }}" onclick="event.preventDefault();
                              document.getElementById('logout-form').submit();" class="list">
